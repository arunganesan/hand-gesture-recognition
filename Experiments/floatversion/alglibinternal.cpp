     ...
     ae_state *_state)
{
    ...
    if( n!=1 )
    {
        ...
        do
        {
            ...
            {
                k = t/2;
                if( ae_fp_greater_eq(buf->ra1.ptr.p_float[k-1],buf->ra1.ptr.p_float[t-1]) )
                {
                    t = 1;
                }
                else
                {
                    tmp = buf->ra1.ptr.p_float[k-1];
                    buf->ra1.ptr.p_float[k-1] = buf->ra1.ptr.p_float[t-1];
                    buf->ra1.ptr.p_float[t-1] = tmp;
        ...
        do
        {
            tmp = buf->ra1.ptr.p_float[i];
            buf->ra1.ptr.p_float[i] = buf->ra1.ptr.p_float[0];
            buf->ra1.ptr.p_float[0] = tmp;
            ...
            while(t!=0)
            {
                ...
                else
                {
                    if( k<i )
                    {
                        if( ae_fp_greater(buf->ra1.ptr.p_float[k],buf->ra1.ptr.p_float[k-1]) )
                        {
                            k = k+1;
                        }
                    }
                    if( ae_fp_greater_eq(buf->ra1.ptr.p_float[t-1],buf->ra1.ptr.p_float[k-1]) )
                    {
                        t = 0;
                    }
                    else
    ...
    while(i<=n-1)
    {
        ...
        for(k=i; k<=j-1; k++)
        {
            buf->ra1.ptr.p_float[k] = 1+(float)(i+j-1)/(float)2;
        }
        i = j;
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_float[buf->ia1.ptr.p_int[i]] = buf->ra1.ptr.p_float[i];
    }
}
     ...
     ae_state *_state)
{
    ...
    for(ix=i1; ix<=i2; ix++)
    {
        if( ae_fp_neq(x->ptr.p_float[ix],0) )
        {
     ...
     ae_state *_state)
{
    ...
    result = i1;
    a = ae_fabs(x->ptr.p_float[result], _state);
    for(i=i1+1; i<=i2; i++)
    {
     ...
     ae_state *_state)
{
    ...
    result = i1;
    a = ae_fabs(x->ptr.pp_float[result][j], _state);
    for(i=i1+1; i<=i2; i++)
    {
     ...
     ae_state *_state)
{
    ...
    result = j1;
    a = ae_fabs(x->ptr.pp_float[i][result], _state);
    for(j=j1+1; j<=j2; j++)
    {
     ...
     ae_state *_state)
{
    ...
    work->ptr.p_float[1] = 0;
    work->ptr.p_float[i] = 0;
    ...
    *info = 0;
    ...
    if( initz )
    {
        ...
        for(i=1; i<=n; i++)
        {
            for(j=1; j<=n; j++)
            {
                if( i==j )
                {
                    z->ptr.pp_float[i][j] = 1;
                }
                else
                {
                    z->ptr.pp_float[i][j] = 0;
                }
            }
        }
    }
    ...
    if( (ns<=2||ns>n)||maxb>=n )
    {
        ...
        if( wantt )
        {
            ...
            while(j<=n)
            {
                if( ae_fp_eq(wi->ptr.p_float[j],0) )
                {
                    for(i=j+1; i<=n; i++)
                    {
                        h->ptr.pp_float[i][j] = 0;
    ...
    for(;;)
    {
        ...
        for(its=0; its<=itn; its++)
        {
            ...
            for(j=i-ns+1; j<=i; j++)
            {
                if( ae_fp_greater_eq(wi->ptr.p_float[j],0) )
                {
                    if( ae_fp_eq(wi->ptr.p_float[j],0) )
                    {
                    ...
                    {
                        if( ae_fp_greater(wi->ptr.p_float[j],0) )
                        {
        ...
        for(its=0; its<=itn; its++)
        {
            ...
            for(k=m; k<=i-1; k++)
            {
                ...
                if( k>m )
                {
                    ...
                    h->ptr.pp_float[k+1][k-1] = 0;
                    if( k<i-1 )
                    {
                        h->ptr.pp_float[k+2][k-1] = 0;
                    }
                }
                else
                {
                    ...
                    else
                    {
                        if( ae_fp_greater(tjj,0) )
                        {
                            ...
                            if( ae_fp_greater(xj,tjj*bignum) )
                            {
                                 ...
                                 */
                                rec = tjj*bignum/xj;
                                if( ae_fp_greater(cnorm->ptr.p_float[j],1) )
                                {
     ...
     ae_state *_state)
{
    ...
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fftrealcooleytukeyplan )
    {
        ...
        for(i=0; i<=n1/2-1; i++)
        {
            ...
            for(k=0; k<=n2-1; k++)
            {
                a->ptr.p_float[offs+2*k+1] = a->ptr.p_float[offs+2*n2+2*k+0];
            }
            ftbaseexecuteplanrec(a, offs, plan, plan->plan.ptr.p_int[entryoffset+6], stackptr, _state);
            plan->tmpbuf.ptr.p_float[0] = a->ptr.p_float[offs+0];
            plan->tmpbuf.ptr.p_float[1] = 0;
            plan->tmpbuf.ptr.p_float[2*n2+0] = a->ptr.p_float[offs+1];
            plan->tmpbuf.ptr.p_float[2*n2+1] = 0;
            for(k=1; k<=n2-1; k++)
            {
                offs1 = 2*k;
                offs2 = 2*n2+2*k;
                hk = a->ptr.p_float[offs+2*k+0];
                hnk = a->ptr.p_float[offs+2*(n2-k)+0];
                plan->tmpbuf.ptr.p_float[offs1+0] = 0.5*(hk+hnk);
                plan->tmpbuf.ptr.p_float[offs2+1] = -0.5*(hk-hnk);
                hk = a->ptr.p_float[offs+2*k+1];
                hnk = a->ptr.p_float[offs+2*(n2-k)+1];
                plan->tmpbuf.ptr.p_float[offs2+0] = 0.5*(hk+hnk);
                plan->tmpbuf.ptr.p_float[offs1+1] = 0.5*(hk-hnk);
            }
            ae_v_move(&a->ptr.p_float[offs], 1, &plan->tmpbuf.ptr.p_float[0], 1, ae_v_len(offs,offs+2*n2*2-1));
        }
        if( n1%2!=0 )
        {
    ...
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fhtcooleytukeyplan )
    {
        ...
        for(i=0; i<=n2-1; i++)
        {
            ...
            {
                offsa = aoffset+i*n1;
                hk = a->ptr.p_float[offsa+j];
                hnk = a->ptr.p_float[offsa+(n1-j)%n1];
                offs = 2*(i*n1+j);
                plan->tmpbuf.ptr.p_float[offs+0] = -0.5*(hnk-hk);
                plan->tmpbuf.ptr.p_float[offs+1] = 0.5*(hk+hnk);
            }
        }
        ftbase_ffttwcalc(&plan->tmpbuf, 0, n1, n2, _state);
        for(j=0; j<=n1-1; j++)
        {
            a->ptr.p_float[aoffset+j] = plan->tmpbuf.ptr.p_float[2*j+0]+plan->tmpbuf.ptr.p_float[2*j+1];
        }
        if( n2%2==0 )
        {
            offs = 2*(n2/2)*n1;
            ...
            for(j=0; j<=n1-1; j++)
            {
                a->ptr.p_float[offsa+j] = plan->tmpbuf.ptr.p_float[offs+2*j+0]+plan->tmpbuf.ptr.p_float[offs+2*j+1];
            }
        }
        for(i=1; i<=(n2+1)/2-1; i++)
        {
            ...
            for(j=0; j<=n1-1; j++)
            {
                a->ptr.p_float[offsa+j] = plan->tmpbuf.ptr.p_float[offs+2*j+1]+plan->tmpbuf.ptr.p_float[offs2+2*j+0];
            }
            offsa = aoffset+(n2-i)*n1;
            for(j=0; j<=n1-1; j++)
            {
                a->ptr.p_float[offsa+j] = plan->tmpbuf.ptr.p_float[offs+2*j+0]+plan->tmpbuf.ptr.p_float[offs2+2*j+1];
            }
        }
    ...
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fftcodeletplan )
    {
        ...
        if( n==3 )
        {
            ...
            c1 = plan->precomputed.ptr.p_float[offs+0];
            c2 = plan->precomputed.ptr.p_float[offs+1];
            a0x = a->ptr.p_float[aoffset+0];
            a0y = a->ptr.p_float[aoffset+1];
            a1x = a->ptr.p_float[aoffset+2];
            a1y = a->ptr.p_float[aoffset+3];
        ...
        if( n==5 )
        {
            ...
            c1 = plan->precomputed.ptr.p_float[offs+0];
            c2 = plan->precomputed.ptr.p_float[offs+1];
            c3 = plan->precomputed.ptr.p_float[offs+2];
            c4 = plan->precomputed.ptr.p_float[offs+3];
            c5 = plan->precomputed.ptr.p_float[offs+4];
    ...
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fhtcodeletplan )
    {
        ...
        {
            a0x = a->ptr.p_float[aoffset+0];
            a1x = a->ptr.p_float[aoffset+1];
            a->ptr.p_float[aoffset+0] = a0x+a1x;
            a->ptr.p_float[aoffset+1] = a0x-a1x;
            ae_frame_leave(_state);
            return;
        }
        if( n==3 )
        {
            ...
            s1x = a0x+m1x;
            a->ptr.p_float[aoffset+0] = a0x;
            a->ptr.p_float[aoffset+1] = s1x-m2y;
            a->ptr.p_float[aoffset+2] = s1x+m2y;
            ae_frame_leave(_state);
            return;
        }
        if( n==4 )
        {
            ...
            a1x = a->ptr.p_float[aoffset+1];
            a2x = a->ptr.p_float[aoffset+2];
            a3x = a->ptr.p_float[aoffset+3];
            t1x = a0x+a2x;
            t2x = a1x+a3x;
            m2x = a0x-a2x;
            m3y = a3x-a1x;
            a->ptr.p_float[aoffset+0] = t1x+t2x;
            a->ptr.p_float[aoffset+1] = m2x-m3y;
            a->ptr.p_float[aoffset+2] = t1x-t2x;
            a->ptr.p_float[aoffset+3] = m2x+m3y;
            ae_frame_leave(_state);
            return;
        }
        if( n==5 )
        {
            ...
            c4 = plan->precomputed.ptr.p_float[offs+3];
            c5 = plan->precomputed.ptr.p_float[offs+4];
            t1x = a->ptr.p_float[aoffset+1]+a->ptr.p_float[aoffset+4];
            t2x = a->ptr.p_float[aoffset+2]+a->ptr.p_float[aoffset+3];
            t3x = a->ptr.p_float[aoffset+1]-a->ptr.p_float[aoffset+4];
            t4x = a->ptr.p_float[aoffset+3]-a->ptr.p_float[aoffset+2];
            t5x = t1x+t2x;
            v0 = a->ptr.p_float[aoffset+0]+t5x;
            a->ptr.p_float[aoffset+0] = v0;
            m2x = c2*(t1x-t2x);
            m3y = c3*(t3x+t4x);
            s3y = m3y-c4*t4x;
            ...
            s2x = s1x+m2x;
            s4x = s1x-m2x;
            a->ptr.p_float[aoffset+1] = s2x-s3y;
            a->ptr.p_float[aoffset+2] = s4x-s5y;
            a->ptr.p_float[aoffset+3] = s4x+s5y;
            a->ptr.p_float[aoffset+4] = s2x+s3y;
            ae_frame_leave(_state);
            return;
        }
    }
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fftbluesteinplan )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            bx = plan->precomputed.ptr.p_float[offsp+0];
            by = plan->precomputed.ptr.p_float[offsp+1];
            x = a->ptr.p_float[offsa+0];
            y = a->ptr.p_float[offsa+1];
            plan->stackbuf.ptr.p_float[offsb+0] = x*bx-y*(-by);
            plan->stackbuf.ptr.p_float[offsb+1] = x*(-by)+y*bx;
            offsp = offsp+2;
            offsa = offsa+2;
            offsb = offsb+2;
        }
        ...
        for(i=0; i<=m-1; i++)
        {
            x = plan->stackbuf.ptr.p_float[offsb+0];
            y = plan->stackbuf.ptr.p_float[offsb+1];
            bx = plan->precomputed.ptr.p_float[offsp+0];
            by = plan->precomputed.ptr.p_float[offsp+1];
            plan->stackbuf.ptr.p_float[offsb+0] = x*bx-y*by;
            plan->stackbuf.ptr.p_float[offsb+1] = -(x*by+y*bx);
        ...
        for(i=0; i<=n-1; i++)
        {
            x = plan->stackbuf.ptr.p_float[offsb+0]/m;
            y = -plan->stackbuf.ptr.p_float[offsb+1]/m;
            bx = plan->precomputed.ptr.p_float[offsp+0];
            by = plan->precomputed.ptr.p_float[offsp+1];
            a->ptr.p_float[offsa+0] = x*bx-y*(-by);
            a->ptr.p_float[offsa+1] = x*(-by)+y*bx;
            offsp = offsp+2;
            offsa = offsa+2;
            offsb = offsb+2;
        }
     ...
     ae_state *_state)
{
    ...
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fftcodeletplan||plan->plan.ptr.p_int[entryoffset+3]==ftbase_fhtcodeletplan )
    {
        ...
        {
            offs = plan->plan.ptr.p_int[entryoffset+7];
            plan->precomputed.ptr.p_float[offs+0] = ae_cos(2*ae_pi/3, _state)-1;
            plan->precomputed.ptr.p_float[offs+1] = ae_sin(2*ae_pi/3, _state);
            ae_frame_leave(_state);
            return;
        }
        if( n==5 )
        {
            offs = plan->plan.ptr.p_int[entryoffset+7];
            v = 2*ae_pi/5;
            plan->precomputed.ptr.p_float[offs+0] = (ae_cos(v, _state)+ae_cos(2*v, _state))/2-1;
            plan->precomputed.ptr.p_float[offs+1] = (ae_cos(v, _state)-ae_cos(2*v, _state))/2;
            plan->precomputed.ptr.p_float[offs+2] = -ae_sin(v, _state);
            plan->precomputed.ptr.p_float[offs+3] = -(ae_sin(v, _state)+ae_sin(2*v, _state));
            plan->precomputed.ptr.p_float[offs+4] = ae_sin(v, _state)-ae_sin(2*v, _state);
            ae_frame_leave(_state);
            return;
        }
    }
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fftbluesteinplan )
    {
        ...
        for(i=0; i<=2*m-1; i++)
        {
            plan->precomputed.ptr.p_float[offs+i] = 0;
        }
        for(i=0; i<=n-1; i++)
        {
            bx = ae_cos(ae_pi*ae_sqr(i, _state)/n, _state);
            by = ae_sin(ae_pi*ae_sqr(i, _state)/n, _state);
            plan->precomputed.ptr.p_float[offs+2*i+0] = bx;
            plan->precomputed.ptr.p_float[offs+2*i+1] = by;
            plan->precomputed.ptr.p_float[offs+2*m+2*i+0] = bx;
            plan->precomputed.ptr.p_float[offs+2*m+2*i+1] = by;
            if( i>0 )
            {
                plan->precomputed.ptr.p_float[offs+2*(m-i)+0] = bx;
                plan->precomputed.ptr.p_float[offs+2*(m-i)+1] = by;
            }
        }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n2-1; i++)
    {
        ...
        for(j=0; j<=n1-1; j++)
        {
            ...
            tmpx = x*twxm1-y*twy;
            tmpy = x*twy+y*twxm1;
            a->ptr.p_float[offs+0] = x+tmpx;
            a->ptr.p_float[offs+1] = y+tmpy;
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&a->ptr.p_float[astart], 1, &buf->ptr.p_float[0], 1, ae_v_len(astart,astart+m*n-1));
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ...
        for(j=0; j<=n-1; j++)
        {
            v = v+a->ptr.p_float[offs+j]*(ae_cos(2*ae_pi*i*j/n, _state)+ae_sin(2*ae_pi*i*j/n, _state));
        }
        buf.ptr.p_float[i] = v;
    }
    for(i=0; i<=n-1; i++)
    {
        a->ptr.p_float[offs+i] = buf.ptr.p_float[i];
    }
    ae_frame_leave(_state);
}
