     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m-2; i++)
    {
        ae_v_move(&c->ptr.pp_float[i+1][i], c->stride, &c->ptr.pp_float[i][i+1], 1, ae_v_len(i+1,m-1));
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m-1; i++)
    {
        t.ptr.p_float[i] = ae_sqrt(c->ptr.pp_float[i][i], _state);
    }
    for(i=0; i<=m-1; i++)
    {
        for(j=0; j<=m-1; j++)
        {
            if( ae_fp_neq(t.ptr.p_float[i],0)&&ae_fp_neq(t.ptr.p_float[j],0) )
            {
                c->ptr.pp_float[i][j] = c->ptr.pp_float[i][j]/(t.ptr.p_float[i]*t.ptr.p_float[j]);
            }
            else
            {
                c->ptr.pp_float[i][j] = 0.0;
            }
        }
    }
    ae_frame_leave(_state);
     ...
     ae_state *_state)
{
    ...
    for(j=0; j<=m-1; j++)
    {
        ae_v_move(&t.ptr.p_float[0], 1, &x->ptr.pp_float[0][j], x->stride, ae_v_len(0,n-1));
        rankx(&t, n, &buf, _state);
        ae_v_move(&x->ptr.pp_float[0][j], x->stride, &t.ptr.p_float[0], 1, ae_v_len(0,n-1));
    }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=m1-1; i++)
    {
        for(j=0; j<=m2-1; j++)
        {
            if( ae_fp_neq(sx.ptr.p_float[i],0)&&ae_fp_neq(sy.ptr.p_float[j],0) )
            {
                c->ptr.pp_float[i][j] = c->ptr.pp_float[i][j]/(sx.ptr.p_float[i]*sy.ptr.p_float[j]);
            }
            else
            {
                c->ptr.pp_float[i][j] = 0;
     ...
     ae_state *_state)
{
    ...
    for(j=0; j<=m2-1; j++)
    {
        ae_v_move(&t.ptr.p_float[0], 1, &y->ptr.pp_float[0][j], y->stride, ae_v_len(0,n-1));
        rankx(&t, n, &buf, _state);
        ae_v_move(&y->ptr.pp_float[0][j], y->stride, &t.ptr.p_float[0], 1, ae_v_len(0,n-1));
    }
