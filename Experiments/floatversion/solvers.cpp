     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    zeroa = ae_fp_eq(sv.ptr.p_float[0],0);
    if( svdfailed||zeroa )
    {
        if( svdfailed )
        {
        ...
        for(i=0; i<=ncols-1; i++)
        {
            for(j=0; j<=ncols-1; j++)
            {
                if( i==j )
                {
                    rep->cx.ptr.pp_float[i][j] = 1;
                }
                else
                {
                    rep->cx.ptr.pp_float[i][j] = 0;
                }
            }
        }
        rep->r2 = 0;
...
lbl_9:
    ...
    for(i=0; i<=n-1; i++)
    {
        state->candstep.ptr.p_float[i] = 0;
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_neq(state->candstep.ptr.p_float[i],0) )
        {
            stepnorm = 1;
            break;
        }
