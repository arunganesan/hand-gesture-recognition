...
need to tune termination conditions for outer iterations too.
  ...
  -- ALGLIB --
     Copyright 28.11.2010 by Bochkanov Sergey
*************************************************************************/
void minbleicsetinnercond(const minbleicstate &state, const float epsg, const float epsf, const float epsx)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
point to the feasible area) at most EpsI.
  ...
  -- ALGLIB --
     Copyright 28.11.2010 by Bochkanov Sergey
*************************************************************************/
void minbleicsetoutercond(const minbleicstate &state, const float epsx, const float epsi)
{
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    try
...
static void mincg_preconditionedmultiply(mincgstate* state,
     ...
     ae_state *_state);
static float mincg_preconditionedmultiply2(mincgstate* state,
     /* Real    */ ae_vector* x,
     /* Real    */ ae_vector* y,
     /* Real    */ ae_vector* work0,
     /* Real    */ ae_vector* work1,
...
static float minbleic_svdtol = 100;
static float minbleic_maxouterits = 20;
...
static float minlm_lambdaup = 2.0;
static float minlm_lambdadown = 0.33;
static float minlm_suspiciousnu = 16;
...
static ae_bool minlm_increaselambda(float* lambdav,
     float* nu,
     ae_state *_state);
static void minlm_decreaselambda(float* lambdav,
     float* nu,
     ae_state *_state);
static float minlm_boundedscaledantigradnorm(minlmstate* state,
     /* Real    */ ae_vector* x,
     /* Real    */ ae_vector* g,
     ae_state *_state);
...
static float mincomp_stpmin = 1.0E-300;
static float mincomp_gtol = 0.3;
static float mincomp_gpaftol = 0.0001;
static float mincomp_gpadecay = 0.5;
static float mincomp_asarho = 0.5;
static float mincomp_asaboundedantigradnorm(minasastate* state,
     ae_state *_state);
static float mincomp_asaginorm(minasastate* state, ae_state *_state);
static float mincomp_asad1norm(minasastate* state, ae_state *_state);
     ...
     ae_state *_state)
{
    ...
    {
        ae_assert(ae_isfinite(s->ptr.p_float[i], _state), "MinCGSetScale: S contains infinite or NAN elements", _state);
        ae_assert(ae_fp_neq(s->ptr.p_float[i],0), "MinCGSetScale: S contains zero elements", _state);
        state->s.ptr.p_float[i] = ae_fabs(s->ptr.p_float[i], _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        ae_assert(ae_isfinite(d->ptr.p_float[i], _state), "MinCGSetPrecDiag: D contains infinite or NAN elements", _state);
        ae_assert(ae_fp_greater(d->ptr.p_float[i],0), "MinCGSetPrecDiag: D contains non-positive elements", _state);
    }
...
ae_bool mincgiteration(mincgstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        betak = state->rstate.ra.ptr.p_float[0];
        v = state->rstate.ra.ptr.p_float[1];
        vv = state->rstate.ra.ptr.p_float[2];
    }
    else
    {
        n = -983;
     ...
     */
    ae_v_move(&state->xk.ptr.p_float[0], 1, &state->x.ptr.p_float[0], 1, ae_v_len(0,n-1));
...
lbl_19:
    ...
    v = state->x.ptr.p_float[i];
    state->x.ptr.p_float[i] = v-state->diffstep*state->s.ptr.p_float[i];
    state->rstate.stage = 2;
    goto lbl_rcomm;
lbl_2:
    state->fm2 = state->f;
    state->x.ptr.p_float[i] = v-0.5*state->diffstep*state->s.ptr.p_float[i];
...
lbl_5:
    state->fp2 = state->f;
    state->x.ptr.p_float[i] = v;
    state->g.ptr.p_float[i] = (8*(state->fp1-state->fm1)-(state->fp2-state->fm2))/(6*state->diffstep*state->s.ptr.p_float[i]);
    i = i+1;
    goto lbl_19;
lbl_21:
    state->f = state->fbase;
...
lbl_22:
    ...
    ae_v_moveneg(&state->dk.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
...
lbl_24:
    ...
    {
        ae_v_move(&state->xn.ptr.p_float[0], 1, &state->xk.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ...
    for(i=0; i<=n-1; i++)
    {
        v = v+ae_sqr(state->g.ptr.p_float[i]*state->s.ptr.p_float[i], _state);
    }
    ...
    if( state->prectype==2||state->prectype==3 )
    {
         ...
         * to the norm of DK
         */
        v = ae_v_dotproduct(&state->dk.ptr.p_float[0], 1, &state->dk.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ...
    else
    {
        ...
        {
            v = ae_v_dotproduct(&state->g.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
            v = ae_sqrt(v, _state);
...
lbl_26:
    ...
    state->terminationneeded = ae_false;
    ae_v_moveneg(&state->yk.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_move(&state->d.ptr.p_float[0], 1, &state->dk.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->xk.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->mcstage = 0;
    state->stp = 1.0;
...
lbl_28:
    if( state->terminationneeded )
    {
        ae_v_move(&state->xn.ptr.p_float[0], 1, &state->x.ptr.p_float[0], 1, ae_v_len(0,n-1));
        state->repterminationtype = 8;
        result = ae_false;
        return result;
    }
...
lbl_39:
    ...
    if( state->mcinfo==1&&!state->innerresetneeded )
    {
         ...
         */
        ae_v_add(&state->yk.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
        vv = ae_v_dotproduct(&state->yk.ptr.p_float[0], 1, &state->dk.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ...
    ae_v_moveneg(&state->dn.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
    mincg_preconditionedmultiply(state, &state->dn, &state->work0, &state->work1, _state);
    ae_v_addd(&state->dn.ptr.p_float[0], 1, &state->dk.ptr.p_float[0], 1, ae_v_len(0,n-1), betak);
    state->laststep = 0;
    state->lastscaledstep = 0.0;
    for(i=0; i<=n-1; i++)
    {
        state->laststep = state->laststep+ae_sqr(state->d.ptr.p_float[i], _state);
        state->lastscaledstep = state->lastscaledstep+ae_sqr(state->d.ptr.p_float[i]/state->s.ptr.p_float[i], _state);
    }
     ...
     */
    ae_v_move(&state->xk.ptr.p_float[0], 1, &state->xn.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_move(&state->dk.ptr.p_float[0], 1, &state->dn.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->fold = state->f;
    state->k = state->k+1;
    goto lbl_26;
lbl_27:
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[0] = betak;
    state->rstate.ra.ptr.p_float[1] = v;
    state->rstate.ra.ptr.p_float[2] = vv;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&x->ptr.p_float[0], 1, &state->xn.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        state->diagh.ptr.p_float[i] = d->ptr.p_float[i];
        state->diaghl2.ptr.p_float[i] = 0.0;
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        state->diagh.ptr.p_float[i] = d1->ptr.p_float[i];
        state->diaghl2.ptr.p_float[i] = 0.0;
    }
    for(i=0; i<=vcnt-1; i++)
    {
        for(j=i; j<=vcnt-1; j++)
        {
            ...
            for(k=0; k<=n-1; k++)
            {
                t = t+v->ptr.pp_float[i][k]*v->ptr.pp_float[j][k]/d1->ptr.p_float[k];
            }
            b.ptr.pp_float[i][j] = t;
        }
        b.ptr.pp_float[i][i] = b.ptr.pp_float[i][i]+1.0/c->ptr.p_float[i];
    }
    ...
    for(i=0; i<=vcnt-1; i++)
    {
        ae_v_move(&state->vcorr.ptr.pp_float[i][0], 1, &v->ptr.pp_float[i][0], 1, ae_v_len(0,n-1));
        for(j=0; j<=i-1; j++)
        {
            t = b.ptr.pp_float[j][i];
            ae_v_subd(&state->vcorr.ptr.pp_float[i][0], 1, &state->vcorr.ptr.pp_float[j][0], 1, ae_v_len(0,n-1), t);
        }
        t = 1/b.ptr.pp_float[i][i];
        ae_v_muld(&state->vcorr.ptr.pp_float[i][0], 1, ae_v_len(0,n-1), t);
    }
    ae_frame_leave(_state);
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        state->diaghl2.ptr.p_float[i] = d2->ptr.p_float[i];
    }
}
     ...
     ae_state *_state)
{
    ...
    if( state->prectype==3 )
    {
        for(i=0; i<=n-1; i++)
        {
            x->ptr.p_float[i] = x->ptr.p_float[i]*state->s.ptr.p_float[i]*state->s.ptr.p_float[i];
        }
        return;
    }
    ...
    for(i=0; i<=n-1; i++)
    {
        x->ptr.p_float[i] = x->ptr.p_float[i]/(state->diagh.ptr.p_float[i]+state->diaghl2.ptr.p_float[i]);
    }
    ...
    if( vcnt>0 )
    {
        for(i=0; i<=vcnt-1; i++)
        {
            v = ae_v_dotproduct(&state->vcorr.ptr.pp_float[i][0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,n-1));
            work0->ptr.p_float[i] = v;
        }
        for(i=0; i<=n-1; i++)
        {
            work1->ptr.p_float[i] = 0;
        }
        for(i=0; i<=vcnt-1; i++)
        {
            v = work0->ptr.p_float[i];
            ae_v_addd(&state->work1.ptr.p_float[0], 1, &state->vcorr.ptr.pp_float[i][0], 1, ae_v_len(0,n-1), v);
        }
        for(i=0; i<=n-1; i++)
        {
            x->ptr.p_float[i] = x->ptr.p_float[i]-state->work1.ptr.p_float[i]/(state->diagh.ptr.p_float[i]+state->diaghl2.ptr.p_float[i]);
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    if( state->prectype==0 )
    {
        v0 = ae_v_dotproduct(&x->ptr.p_float[0], 1, &y->ptr.p_float[0], 1, ae_v_len(0,n-1));
        result = v0;
        return result;
    }
    if( state->prectype==3 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            result = result+x->ptr.p_float[i]*state->s.ptr.p_float[i]*state->s.ptr.p_float[i]*y->ptr.p_float[i];
        }
        return result;
    }
    ae_assert(state->prectype==2, "MinCG: internal error (unexpected PrecType)", _state);
    ...
    for(i=0; i<=n-1; i++)
    {
        result = result+x->ptr.p_float[i]*y->ptr.p_float[i]/(state->diagh.ptr.p_float[i]+state->diaghl2.ptr.p_float[i]);
    }
    if( vcnt>0 )
    {
        for(i=0; i<=n-1; i++)
        {
            work0->ptr.p_float[i] = x->ptr.p_float[i]/(state->diagh.ptr.p_float[i]+state->diaghl2.ptr.p_float[i]);
            work1->ptr.p_float[i] = y->ptr.p_float[i]/(state->diagh.ptr.p_float[i]+state->diaghl2.ptr.p_float[i]);
        }
        for(i=0; i<=vcnt-1; i++)
        {
            v0 = ae_v_dotproduct(&work0->ptr.p_float[0], 1, &state->vcorr.ptr.pp_float[i][0], 1, ae_v_len(0,n-1));
            v1 = ae_v_dotproduct(&work1->ptr.p_float[0], 1, &state->vcorr.ptr.pp_float[i][0], 1, ae_v_len(0,n-1));
            result = result-v0*v1;
        }
    }
    return result;
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ae_assert(ae_isfinite(bndl->ptr.p_float[i], _state)||ae_isneginf(bndl->ptr.p_float[i], _state), "MinBLEICSetBC: BndL contains NAN or +INF", _state);
        ae_assert(ae_isfinite(bndu->ptr.p_float[i], _state)||ae_isposinf(bndu->ptr.p_float[i], _state), "MinBLEICSetBC: BndL contains NAN or -INF", _state);
        state->bndloriginal.ptr.p_float[i] = bndl->ptr.p_float[i];
        state->hasbndl.ptr.p_bool[i] = ae_isfinite(bndl->ptr.p_float[i], _state);
        state->bnduoriginal.ptr.p_float[i] = bndu->ptr.p_float[i];
        state->hasbndu.ptr.p_bool[i] = ae_isfinite(bndu->ptr.p_float[i], _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=k-1; i++)
    {
        ...
        ae_v_move(&state->ceoriginal.ptr.pp_float[i][0], 1, &c->ptr.pp_float[i][0], 1, ae_v_len(0,nmain));
    }
}
     ...
     ae_state *_state)
{
    ...
    {
        ae_assert(ae_isfinite(s->ptr.p_float[i], _state), "MinBLEICSetScale: S contains infinite or NAN elements", _state);
        ae_assert(ae_fp_neq(s->ptr.p_float[i],0), "MinBLEICSetScale: S contains zero elements", _state);
        state->soriginal.ptr.p_float[i] = ae_fabs(s->ptr.p_float[i], _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->nmain-1; i++)
    {
        ae_assert(ae_isfinite(d->ptr.p_float[i], _state), "MinBLEICSetPrecDiag: D contains infinite or NAN elements", _state);
        ae_assert(ae_fp_greater(d->ptr.p_float[i],0), "MinBLEICSetPrecDiag: D contains non-positive elements", _state);
    }
    rvectorsetlengthatleast(&state->diaghoriginal, state->nmain, _state);
    state->prectype = 2;
    for(i=0; i<=state->nmain-1; i++)
    {
        state->diaghoriginal.ptr.p_float[i] = d->ptr.p_float[i];
    }
}
...
ae_bool minbleiciteration(minbleicstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        vv = state->rstate.ra.ptr.p_float[1];
    }
    else
    {
        nmain = -983;
    ...
    for(i=0; i<=nmain-1; i++)
    {
        if( state->prectype==2 )
        {
            state->transforms.ptr.p_float[i] = 1/ae_sqrt(state->diaghoriginal.ptr.p_float[i], _state);
            continue;
        }
        if( state->prectype==3 )
        {
            state->transforms.ptr.p_float[i] = state->soriginal.ptr.p_float[i];
            continue;
        }
        state->transforms.ptr.p_float[i] = 1;
    }
    rvectorsetlengthatleast(&state->seffective, nmain+nslack, _state);
    for(i=0; i<=nmain-1; i++)
    {
        state->seffective.ptr.p_float[i] = state->soriginal.ptr.p_float[i]/state->transforms.ptr.p_float[i];
    }
    for(i=0; i<=nslack-1; i++)
    {
        state->seffective.ptr.p_float[nmain+i] = 1;
    }
    ...
    for(i=0; i<=nmain-1; i++)
    {
        ...
        {
            state->bndleffective.ptr.p_float[i] = state->bndloriginal.ptr.p_float[i]/state->transforms.ptr.p_float[i];
        }
        if( state->hasbndu.ptr.p_bool[i] )
        {
            state->bndueffective.ptr.p_float[i] = state->bnduoriginal.ptr.p_float[i]/state->transforms.ptr.p_float[i];
        }
    }
    for(i=0; i<=nmain-1; i++)
    {
        ...
        {
            if( ae_fp_greater(state->bndleffective.ptr.p_float[i],state->bndueffective.ptr.p_float[i]) )
            {
    ...
    for(i=0; i<=state->cecnt-1; i++)
    {
        ...
        for(j=0; j<=nmain-1; j++)
        {
            state->ceeffective.ptr.pp_float[i][j] = state->ceoriginal.ptr.pp_float[i][j]*state->transforms.ptr.p_float[j];
            v = ae_maxreal(v, ae_fabs(state->ceeffective.ptr.pp_float[i][j], _state), _state);
        }
        if( ae_fp_eq(v,0) )
        {
            v = 1;
        ...
        {
            state->ceeffective.ptr.pp_float[i][nmain+j] = 0.0;
        }
        state->ceeffective.ptr.pp_float[i][nmain+nslack] = state->ceoriginal.ptr.pp_float[i][nmain];
        if( state->ct.ptr.p_int[i]<0 )
        {
            state->ceeffective.ptr.pp_float[i][nmain+m] = v;
            m = m+1;
        }
        if( state->ct.ptr.p_int[i]>0 )
        {
    ...
    for(i=0; i<=nmain-1; i++)
    {
        state->tmp0.ptr.p_float[i] = state->xstart.ptr.p_float[i]/state->transforms.ptr.p_float[i];
    }
    m = 0;
    for(i=0; i<=state->cecnt-1; i++)
    {
        v = ae_v_dotproduct(&state->ceeffective.ptr.pp_float[i][0], 1, &state->tmp0.ptr.p_float[0], 1, ae_v_len(0,nmain-1));
        if( state->ct.ptr.p_int[i]<0 )
        {
            state->tmp0.ptr.p_float[nmain+m] = state->ceeffective.ptr.pp_float[i][nmain+nslack]-v;
            m = m+1;
        }
        if( state->ct.ptr.p_int[i]>0 )
        {
            state->tmp0.ptr.p_float[nmain+m] = v-state->ceeffective.ptr.pp_float[i][nmain+nslack];
            m = m+1;
        }
    }
    for(i=0; i<=nmain+nslack-1; i++)
    {
        state->tmp1.ptr.p_float[i] = 0;
    }
    for(i=0; i<=nmain+nslack-1; i++)
    {
    ...
    for(i=0; i<=state->cecnt-1; i++)
    {
        v = ae_v_dotproduct(&state->ceeffective.ptr.pp_float[i][0], 1, &state->xcur.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
        state->repdebugeqerr = state->repdebugeqerr+ae_sqr(v-state->ceeffective.ptr.pp_float[i][nmain+nslack], _state);
    }
...
lbl_14:
    ...
    ae_v_move(&state->xprev.ptr.p_float[0], 1, &state->xcur.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
lbl_15:
    if( ae_false )
    {
        goto lbl_16;
     ...
     */
    ae_v_move(&state->tmp0.ptr.p_float[0], 1, &state->xcur.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
    for(i=0; i<=nmain+nslack-1; i++)
    {
        state->tmp1.ptr.p_float[i] = 0;
        state->activeconstraints.ptr.p_bool[i] = ae_false;
...
lbl_17:
    ...
    if( state->cgstate.algpowerup )
    {
        ...
        {
            minbleic_rebuildcexe(state, _state);
            ae_v_move(&state->tmp1.ptr.p_float[0], 1, &state->cgstate.g.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
            ...
            for(i=0; i<=nmain-1; i++)
            {
                if( !state->activeconstraints.ptr.p_bool[i] )
                {
                    if( state->hasbndl.ptr.p_bool[i] )
                    {
                        if( ae_fp_eq(state->cgstate.x.ptr.p_float[i],state->bndleffective.ptr.p_float[i])&&ae_fp_greater_eq(state->tmp1.ptr.p_float[i],0) )
                        {
                            state->activeconstraints.ptr.p_bool[i] = ae_true;
                            state->constrainedvalues.ptr.p_float[i] = state->bndleffective.ptr.p_float[i];
                            b = ae_true;
                        }
                    }
                    if( state->hasbndu.ptr.p_bool[i] )
                    {
                        if( ae_fp_eq(state->cgstate.x.ptr.p_float[i],state->bndueffective.ptr.p_float[i])&&ae_fp_less_eq(state->tmp1.ptr.p_float[i],0) )
                        {
                            state->activeconstraints.ptr.p_bool[i] = ae_true;
                            state->constrainedvalues.ptr.p_float[i] = state->bndueffective.ptr.p_float[i];
                            b = ae_true;
                        }
                    }
                }
            ...
            for(i=0; i<=nslack-1; i++)
            {
                ...
                {
                    if( ae_fp_eq(state->cgstate.x.ptr.p_float[nmain+i],0)&&ae_fp_greater_eq(state->tmp1.ptr.p_float[nmain+i],0) )
                    {
                        state->activeconstraints.ptr.p_bool[nmain+i] = ae_true;
                        state->constrainedvalues.ptr.p_float[nmain+i] = 0;
                        b = ae_true;
                    }
                }
            }
        ...
        while(b);
        ae_v_move(&state->cgstate.g.ptr.p_float[0], 1, &state->tmp1.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
        goto lbl_17;
    }
    if( state->cgstate.lsstart )
    {
        ...
        for(i=0; i<=nmain-1; i++)
        {
            if( state->hasbndl.ptr.p_bool[i]&&ae_fp_less(state->cgstate.d.ptr.p_float[i],0) )
            {
                v = state->cgstate.curstpmax;
                vv = state->cgstate.x.ptr.p_float[i]-state->bndleffective.ptr.p_float[i];
                if( ae_fp_less(vv,0) )
                {
                    vv = 0;
                }
                state->cgstate.curstpmax = safeminposrv(vv, -state->cgstate.d.ptr.p_float[i], state->cgstate.curstpmax, _state);
                if( ae_fp_less(state->cgstate.curstpmax,v) )
                {
                    state->variabletofreeze = i;
                    state->valuetofreeze = state->bndleffective.ptr.p_float[i];
                }
            }
            if( state->hasbndu.ptr.p_bool[i]&&ae_fp_greater(state->cgstate.d.ptr.p_float[i],0) )
            {
                v = state->cgstate.curstpmax;
                vv = state->bndueffective.ptr.p_float[i]-state->cgstate.x.ptr.p_float[i];
                if( ae_fp_less(vv,0) )
                {
                    vv = 0;
                }
                ...
                {
                    state->variabletofreeze = i;
                    state->valuetofreeze = state->bndueffective.ptr.p_float[i];
                }
            }
        }
        for(i=0; i<=nslack-1; i++)
        {
            if( ae_fp_less(state->cgstate.d.ptr.p_float[nmain+i],0) )
            {
                v = state->cgstate.curstpmax;
                vv = state->cgstate.x.ptr.p_float[nmain+i];
                if( ae_fp_less(vv,0) )
                {
                    vv = 0;
                }
                state->cgstate.curstpmax = safeminposrv(vv, -state->cgstate.d.ptr.p_float[nmain+i], state->cgstate.curstpmax, _state);
                if( ae_fp_less(state->cgstate.curstpmax,v) )
                {
        ...
        if( ae_fp_eq(state->cgstate.curstpmax,0) )
        {
            ...
            state->constrainedvalues.ptr.p_float[state->variabletofreeze] = state->valuetofreeze;
            state->cgstate.x.ptr.p_float[state->variabletofreeze] = state->valuetofreeze;
    ...
    if( state->cgstate.lsend )
    {
        ...
        if( b )
        {
            ae_v_move(&state->tmp0.ptr.p_float[0], 1, &state->cgstate.x.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
            ae_v_move(&state->tmp1.ptr.p_float[0], 1, &state->lastg.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
    ...
    ae_v_move(&state->tmp1.ptr.p_float[0], 1, &state->cgstate.x.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
...
lbl_23:
    ...
    if( state->hasbndl.ptr.p_bool[i] )
    {
        b = b||ae_fp_less(v-state->diffstep*state->soriginal.ptr.p_float[i],state->bndloriginal.ptr.p_float[i]);
    }
    if( state->hasbndu.ptr.p_bool[i] )
    {
        b = b||ae_fp_greater(v+state->diffstep*state->soriginal.ptr.p_float[i],state->bnduoriginal.ptr.p_float[i]);
    }
    if( b )
    {
        goto lbl_26;
    }
    state->x.ptr.p_float[i] = v-state->diffstep*state->soriginal.ptr.p_float[i];
    state->rstate.stage = 4;
    goto lbl_rcomm;
lbl_4:
    state->fm2 = state->f;
    state->x.ptr.p_float[i] = v-0.5*state->diffstep*state->soriginal.ptr.p_float[i];
    state->rstate.stage = 5;
    goto lbl_rcomm;
lbl_5:
    state->fm1 = state->f;
...
lbl_7:
    state->fp2 = state->f;
    state->g.ptr.p_float[i] = (8*(state->fp1-state->fm1)-(state->fp2-state->fm2))/(6*state->diffstep*state->soriginal.ptr.p_float[i]);
    goto lbl_27;
lbl_26:
    state->xm1 = ae_maxreal(v-state->diffstep*state->soriginal.ptr.p_float[i], state->bndloriginal.ptr.p_float[i], _state);
    state->x.ptr.p_float[i] = state->xm1;
    state->rstate.stage = 8;
    goto lbl_rcomm;
lbl_8:
    state->fm1 = state->f;
    state->xp1 = ae_minreal(v+state->diffstep*state->soriginal.ptr.p_float[i], state->bnduoriginal.ptr.p_float[i], _state);
    state->x.ptr.p_float[i] = state->xp1;
    state->rstate.stage = 9;
    goto lbl_rcomm;
lbl_9:
    state->fp1 = state->f;
    state->g.ptr.p_float[i] = (state->fp1-state->fm1)/(state->xp1-state->xm1);
lbl_27:
    state->x.ptr.p_float[i] = v;
    i = i+1;
    goto lbl_23;
...
lbl_22:
    if( ae_fp_less(state->f,state->trimthreshold) )
    {
        ...
        ae_v_move(&state->lastg.ptr.p_float[0], 1, &state->cgstate.g.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
    ...
    else
    {
        ...
        for(i=0; i<=nmain+nslack-1; i++)
        {
            state->cgstate.g.ptr.p_float[i] = 0.0;
        }
    }
    goto lbl_17;
lbl_19:
...
lbl_31:
    ...
    for(i=0; i<=nmain-1; i++)
    {
        v = v+ae_sqr((state->xcur.ptr.p_float[i]-state->xprev.ptr.p_float[i])/state->seffective.ptr.p_float[i], _state);
    }
    v = ae_sqrt(v, _state);
...
lbl_16:
    ...
    for(i=0; i<=nmain-1; i++)
    {
        state->repdebugdx = state->repdebugdx+ae_sqr(state->xcur.ptr.p_float[i]-state->xstart.ptr.p_float[i], _state);
    }
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[1] = vv;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    {
        ae_v_move(&x->ptr.p_float[0], 1, &state->xend.ptr.p_float[0], 1, ae_v_len(0,state->nmain-1));
    }
    else
    {
        for(i=0; i<=state->nmain-1; i++)
        {
            x->ptr.p_float[i] = _state->v_nan;
        }
    }
     ...
     ae_state *_state)
{
     ...
     * Set XC
     */
    ae_v_move(&state->xstart.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,n-1));
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->nmain-1; i++)
    {
        v = xscaled->ptr.p_float[i]*state->transforms.ptr.p_float[i];
        if( state->hasbndl.ptr.p_bool[i] )
        {
            if( ae_fp_less(v,state->bndloriginal.ptr.p_float[i]) )
            {
                v = state->bndloriginal.ptr.p_float[i];
            }
        }
        if( state->hasbndu.ptr.p_bool[i] )
        {
            if( ae_fp_greater(v,state->bnduoriginal.ptr.p_float[i]) )
            {
                v = state->bnduoriginal.ptr.p_float[i];
            }
        }
        xunscaled->ptr.p_float[i] = v;
    }
}
     ...
     ae_state *_state)
{
    ...
    *rnorm2 = 0;
     ...
     * * unscale variables
     */
    ae_v_sub(&xscaled->ptr.p_float[0], 1, &state->xe.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
    *rnorm2 = 0;
    for(i=0; i<=nmain+nslack-1; i++)
    {
        rscaled->ptr.p_float[i] = 0;
    }
    for(i=0; i<=nmain+nslack-1; i++)
    {
        if( state->activeconstraints.ptr.p_bool[i] )
        {
            v = xscaled->ptr.p_float[i];
            xscaled->ptr.p_float[i] = 0;
            rscaled->ptr.p_float[i] = rscaled->ptr.p_float[i]+v;
            *rnorm2 = *rnorm2+ae_sqr(v, _state);
        }
    }
    for(i=0; i<=state->cecnt-1; i++)
    {
        v = ae_v_dotproduct(&xscaled->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
        ae_v_subd(&xscaled->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1), v);
        ae_v_addd(&rscaled->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1), v);
        *rnorm2 = *rnorm2+ae_sqr(v, _state);
    }
    ae_v_add(&xscaled->ptr.p_float[0], 1, &state->xe.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->nmain-1; i++)
    {
        gscaled->ptr.p_float[i] = gunscaled->ptr.p_float[i]*state->transforms.ptr.p_float[i];
    }
    for(i=0; i<=state->nslack-1; i++)
    {
        gscaled->ptr.p_float[state->nmain+i] = 0;
    }
}
     ...
     ae_state *_state)
{
    ...
    *mpgnorm = 0;
     ...
     * GNorm
     */
    v = ae_v_dotproduct(&g->ptr.p_float[0], 1, &g->ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
    ...
    *f = *f+rnorm2;
    ...
    for(i=0; i<=state->cecnt-1; i++)
    {
        v = ae_v_dotproduct(&g->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
        ae_v_subd(&g->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1), v);
    }
    ae_v_addd(&g->ptr.p_float[0], 1, &r->ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1), 2);
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=nmain-1; i++)
    {
        if( !state->activeconstraints.ptr.p_bool[i] )
        {
            if( state->hasbndl.ptr.p_bool[i] )
            {
                if( ae_fp_less_eq(x->ptr.p_float[i],state->bndleffective.ptr.p_float[i]) )
                {
                    state->activeconstraints.ptr.p_bool[i] = ae_true;
                    state->constrainedvalues.ptr.p_float[i] = state->bndleffective.ptr.p_float[i];
                    result = ae_true;
            ...
            if( state->hasbndu.ptr.p_bool[i] )
            {
                if( ae_fp_greater_eq(x->ptr.p_float[i],state->bndueffective.ptr.p_float[i]) )
                {
                    state->activeconstraints.ptr.p_bool[i] = ae_true;
                    state->constrainedvalues.ptr.p_float[i] = state->bndueffective.ptr.p_float[i];
                    result = ae_true;
    ...
    for(i=0; i<=nslack-1; i++)
    {
        if( !state->activeconstraints.ptr.p_bool[nmain+i] )
        {
            if( ae_fp_less_eq(x->ptr.p_float[nmain+i],0) )
            {
                state->activeconstraints.ptr.p_bool[nmain+i] = ae_true;
                state->constrainedvalues.ptr.p_float[nmain+i] = 0;
                result = ae_true;
...
static void minbleic_rebuildcexe(minbleicstate* state, ae_state *_state)
{
    ...
    for(i=0; i<=state->cecnt-1; i++)
    {
        ...
        for(j=0; j<=nmain+nslack-1; j++)
        {
            ...
            {
                state->cecurrent.ptr.pp_float[i][nmain+nslack] = state->cecurrent.ptr.pp_float[i][nmain+nslack]-state->cecurrent.ptr.pp_float[i][j]*state->constrainedvalues.ptr.p_float[j];
                state->cecurrent.ptr.pp_float[i][j] = 0.0;
            }
        }
        ...
        for(k=0; k<=i-1; k++)
        {
            v = ae_v_dotproduct(&state->cecurrent.ptr.pp_float[k][0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
            ae_v_subd(&state->cecurrent.ptr.pp_float[i][0], 1, &state->cecurrent.ptr.pp_float[k][0], 1, ae_v_len(0,nmain+nslack), v);
        }
         ...
         */
        v = ae_v_dotproduct(&state->cecurrent.ptr.pp_float[i][0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
        v = ae_sqrt(v, _state);
        if( ae_fp_greater(v,10000*ae_machineepsilon) )
        {
            v = 1/v;
            ae_v_muld(&state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack), v);
        }
        else
        {
            for(j=0; j<=nmain+nslack; j++)
            {
                state->cecurrent.ptr.pp_float[i][j] = 0;
            }
        }
    }
    for(j=0; j<=nmain+nslack-1; j++)
    {
        state->xe.ptr.p_float[j] = 0;
    }
    for(i=0; i<=nmain+nslack-1; i++)
    {
        if( state->activeconstraints.ptr.p_bool[i] )
        {
            state->xe.ptr.p_float[i] = state->xe.ptr.p_float[i]+state->constrainedvalues.ptr.p_float[i];
        }
    }
    for(i=0; i<=state->cecnt-1; i++)
    {
        v = state->cecurrent.ptr.pp_float[i][nmain+nslack];
        ae_v_addd(&state->xe.ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1), v);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->cecnt-1; i++)
    {
        v = ae_v_dotproduct(&pg->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
        ae_v_subd(&pg->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1), v);
    }
}
     ...
     ae_state *_state)
{
    ...
    do
    {
         ...
         * Calculate PX, PG
         */
        ae_v_move(&px->ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
        ae_v_sub(&px->ptr.p_float[0], 1, &state->xe.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
        ae_v_move(&pg->ptr.p_float[0], 1, &g->ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
        for(i=0; i<=nmain+nslack-1; i++)
        {
            if( state->activeconstraints.ptr.p_bool[i] )
            {
        ...
        for(i=0; i<=state->cecnt-1; i++)
        {
            v = ae_v_dotproduct(&px->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
            ae_v_subd(&px->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1), v);
            v = ae_v_dotproduct(&pg->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
            ae_v_subd(&pg->ptr.p_float[0], 1, &state->cecurrent.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1), v);
        }
        ae_v_add(&px->ptr.p_float[0], 1, &state->xe.ptr.p_float[0], 1, ae_v_len(0,nmain+nslack-1));
    ...
    for(i=0; i<=state->cecnt-1; i++)
    {
        v = ae_v_dotproduct(&px->ptr.p_float[0], 1, &state->ceeffective.ptr.pp_float[i][0], 1, ae_v_len(0,nmain+nslack-1));
        v = v-state->ceeffective.ptr.pp_float[i][nmain+nslack];
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        state->bndloriginal.ptr.p_float[i] = _state->v_neginf;
        state->hasbndl.ptr.p_bool[i] = ae_false;
        state->bnduoriginal.ptr.p_float[i] = _state->v_posinf;
        state->hasbndu.ptr.p_bool[i] = ae_false;
        state->soriginal.ptr.p_float[i] = 1.0;
    }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        ae_assert(ae_isfinite(s->ptr.p_float[i], _state), "MinLBFGSSetScale: S contains infinite or NAN elements", _state);
        ae_assert(ae_fp_neq(s->ptr.p_float[i],0), "MinLBFGSSetScale: S contains zero elements", _state);
        state->s.ptr.p_float[i] = ae_fabs(s->ptr.p_float[i], _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        mx = ae_maxreal(mx, ae_fabs(p->ptr.pp_float[i][i], _state), _state);
    }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        ae_assert(ae_isfinite(d->ptr.p_float[i], _state), "MinLBFGSSetPrecDiag: D contains infinite or NAN elements", _state);
        ae_assert(ae_fp_greater(d->ptr.p_float[i],0), "MinLBFGSSetPrecDiag: D contains non-positive elements", _state);
    }
    rvectorsetlengthatleast(&state->diagh, state->n, _state);
    state->prectype = 2;
    for(i=0; i<=state->n-1; i++)
...
lbl_19:
     ...
     * Apply preconditioner, if we have something other than default.
     */
    ae_v_moveneg(&state->d.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
    if( state->prectype==0 )
    {
    ...
    if( state->prectype==2 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            state->d.ptr.p_float[i] = state->d.ptr.p_float[i]/state->diagh.ptr.p_float[i];
        }
        state->stp = 1;
    }
    if( state->prectype==3 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            state->d.ptr.p_float[i] = state->d.ptr.p_float[i]*state->s.ptr.p_float[i]*state->s.ptr.p_float[i];
        }
        state->stp = 1;
    }
...
lbl_21:
     ...
     * Store X[k], G[k]
     */
    ae_v_moveneg(&state->sk.ptr.pp_float[state->p][0], 1, &state->x.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_moveneg(&state->yk.ptr.pp_float[state->p][0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
...
lbl_30:
    state->repnfev = state->repnfev+state->nfev;
    state->repiterationscount = state->repiterationscount+1;
    ae_v_add(&state->sk.ptr.pp_float[state->p][0], 1, &state->x.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_add(&state->yk.ptr.pp_float[state->p][0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ...
    for(i=0; i<=n-1; i++)
    {
        v = v+ae_sqr(state->sk.ptr.pp_float[state->p][i]/state->s.ptr.p_float[i], _state);
    }
    ...
    else
    {
         ...
         */
        v = ae_v_dotproduct(&state->yk.ptr.pp_float[state->p][0], 1, &state->sk.ptr.pp_float[state->p][0], 1, ae_v_len(0,n-1));
        vv = ae_v_dotproduct(&state->yk.ptr.pp_float[state->p][0], 1, &state->yk.ptr.pp_float[state->p][0], 1, ae_v_len(0,n-1));
        ...
        state->rho.ptr.p_float[state->p] = 1/v;
        state->gammak = v/vv;
         ...
         */
        ae_v_move(&state->work.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
        for(i=state->k; i>=state->k-state->q; i--)
        {
            ic = i%m;
            v = ae_v_dotproduct(&state->sk.ptr.pp_float[ic][0], 1, &state->work.ptr.p_float[0], 1, ae_v_len(0,n-1));
            state->theta.ptr.p_float[ic] = v;
            vv = v*state->rho.ptr.p_float[ic];
            ae_v_subd(&state->work.ptr.p_float[0], 1, &state->yk.ptr.pp_float[ic][0], 1, ae_v_len(0,n-1), vv);
        }
        if( state->prectype==0 )
        {
             ...
             */
            v = state->gammak;
            ae_v_muld(&state->work.ptr.p_float[0], 1, ae_v_len(0,n-1), v);
        }
        if( state->prectype==1 )
        {
        ...
        if( state->prectype==2 )
        {
            ...
            for(i=0; i<=n-1; i++)
            {
                state->work.ptr.p_float[i] = state->work.ptr.p_float[i]/state->diagh.ptr.p_float[i];
            }
        }
        if( state->prectype==3 )
        {
            ...
            for(i=0; i<=n-1; i++)
            {
                state->work.ptr.p_float[i] = state->work.ptr.p_float[i]*state->s.ptr.p_float[i]*state->s.ptr.p_float[i];
            }
        }
        for(i=state->k-state->q; i<=state->k; i++)
        {
            ic = i%m;
            v = ae_v_dotproduct(&state->yk.ptr.pp_float[ic][0], 1, &state->work.ptr.p_float[0], 1, ae_v_len(0,n-1));
            vv = state->rho.ptr.p_float[ic]*(-v+state->theta.ptr.p_float[ic]);
            ae_v_addd(&state->work.ptr.p_float[0], 1, &state->sk.ptr.pp_float[ic][0], 1, ae_v_len(0,n-1), vv);
        }
        ae_v_moveneg(&state->d.ptr.p_float[0], 1, &state->work.ptr.p_float[0], 1, ae_v_len(0,n-1));
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&x->ptr.p_float[0], 1, &state->x.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    rep->iterationscount = state->repiterationscount;
    rep->nfev = state->repnfev;
    rep->terminationtype = state->repterminationtype;
}
...
void minqpcreate(ae_int_t n, minqpstate* state, ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        state->b.ptr.p_float[i] = 0.0;
        state->workbndl.ptr.p_float[i] = _state->v_neginf;
        state->workbndu.ptr.p_float[i] = _state->v_posinf;
        state->havebndl.ptr.p_bool[i] = ae_false;
        state->havebndu.ptr.p_bool[i] = ae_false;
        state->startx.ptr.p_float[i] = 0.0;
        state->xorigin.ptr.p_float[i] = 0.0;
    }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ae_assert(ae_isfinite(bndl->ptr.p_float[i], _state)||ae_isneginf(bndl->ptr.p_float[i], _state), "MinQPSetBC: BndL contains NAN or +INF", _state);
        ae_assert(ae_isfinite(bndu->ptr.p_float[i], _state)||ae_isposinf(bndu->ptr.p_float[i], _state), "MinQPSetBC: BndU contains NAN or -INF", _state);
        state->bndl.ptr.p_float[i] = bndl->ptr.p_float[i];
        state->havebndl.ptr.p_bool[i] = ae_isfinite(bndl->ptr.p_float[i], _state);
        state->bndu.ptr.p_float[i] = bndu->ptr.p_float[i];
        state->havebndu.ptr.p_bool[i] = ae_isfinite(bndu->ptr.p_float[i], _state);
    }
}
...
void minqpoptimize(minqpstate* state, ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ...
        {
            if( ae_fp_greater(state->bndl.ptr.p_float[i],state->bndu.ptr.p_float[i]) )
            {
                state->repterminationtype = -3;
                return;
            }
    ...
    for(i=0; i<=n-1; i++)
    {
        state->xc.ptr.p_float[i] = state->startx.ptr.p_float[i]-state->xorigin.ptr.p_float[i];
        if( state->havebndl.ptr.p_bool[i] )
        {
            state->workbndl.ptr.p_float[i] = state->bndl.ptr.p_float[i]-state->xorigin.ptr.p_float[i];
        }
        if( state->havebndu.ptr.p_bool[i] )
        {
            state->workbndu.ptr.p_float[i] = state->bndu.ptr.p_float[i]-state->xorigin.ptr.p_float[i];
        }
    }
    ...
    if( state->havex )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            if( state->havebndl.ptr.p_bool[i] )
            {
                if( ae_fp_less(state->xc.ptr.p_float[i],state->workbndl.ptr.p_float[i]) )
                {
                    state->xc.ptr.p_float[i] = state->workbndl.ptr.p_float[i];
                }
            }
            if( state->havebndu.ptr.p_bool[i] )
            {
                if( ae_fp_greater(state->xc.ptr.p_float[i],state->workbndu.ptr.p_float[i]) )
                {
                    state->xc.ptr.p_float[i] = state->workbndu.ptr.p_float[i];
                }
            }
        }
    }
    else
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            if( state->havebndl.ptr.p_bool[i]&&state->havebndu.ptr.p_bool[i] )
            {
                state->xc.ptr.p_float[i] = 0.5*(state->workbndl.ptr.p_float[i]+state->workbndu.ptr.p_float[i]);
                if( ae_fp_less(state->xc.ptr.p_float[i],state->workbndl.ptr.p_float[i]) )
                {
                    state->xc.ptr.p_float[i] = state->workbndl.ptr.p_float[i];
                }
    ...
    if( state->algokind==1&&state->akind==0 )
    {
        ...
        if( nbc==0&&nlc==0 )
        {
            ...
            state->densea.ptr.pp_float[0][0] = state->diaga.ptr.p_float[0];
            for(k=1; k<=n-1; k++)
            {
                ae_v_move(&state->densea.ptr.pp_float[0][k], state->densea.stride, &state->densea.ptr.pp_float[k][0], 1, ae_v_len(0,k-1));
                state->densea.ptr.pp_float[k][k] = state->diaga.ptr.p_float[k];
            }
            ae_v_move(&state->bufb.ptr.p_float[0], 1, &state->b.ptr.p_float[0], 1, ae_v_len(0,n-1));
            ...
            ae_v_moveneg(&state->xc.ptr.p_float[0], 1, &state->bufb.ptr.p_float[0], 1, ae_v_len(0,n-1));
            ae_v_add(&state->xc.ptr.p_float[0], 1, &state->xorigin.ptr.p_float[0], 1, ae_v_len(0,n-1));
        ...
        for(;;)
        {
            ...
            for(i=0; i<=n-1; i++)
            {
                ...
                if( state->havebndl.ptr.p_bool[i] )
                {
                    if( ae_fp_less_eq(state->xc.ptr.p_float[i],state->workbndl.ptr.p_float[i])&&ae_fp_greater_eq(state->gc.ptr.p_float[i],0) )
                    {
                ...
                if( state->havebndu.ptr.p_bool[i] )
                {
                    if( ae_fp_greater_eq(state->xc.ptr.p_float[i],state->workbndu.ptr.p_float[i])&&ae_fp_less_eq(state->gc.ptr.p_float[i],0) )
                    {
                        state->activeconstraints.ptr.p_int[i] = 1;
                    }
                }
                if( state->havebndl.ptr.p_bool[i]&&state->havebndu.ptr.p_bool[i] )
                {
                    if( ae_fp_eq(state->workbndl.ptr.p_float[i],state->workbndu.ptr.p_float[i]) )
                    {
                        state->activeconstraints.ptr.p_int[i] = 1;
                    }
                }
             ...
             */
            state->bufa.ptr.pp_float[0][0] = state->diaga.ptr.p_float[0];
            for(k=1; k<=n-1; k++)
            {
                ae_v_move(&state->bufa.ptr.pp_float[k][0], 1, &state->densea.ptr.pp_float[k][0], 1, ae_v_len(0,k-1));
                ae_v_move(&state->bufa.ptr.pp_float[0][k], state->bufa.stride, &state->densea.ptr.pp_float[k][0], 1, ae_v_len(0,k-1));
                state->bufa.ptr.pp_float[k][k] = state->diaga.ptr.p_float[k];
            }
            ae_v_move(&state->bufb.ptr.p_float[0], 1, &state->b.ptr.p_float[0], 1, ae_v_len(0,n-1));
            ae_v_move(&state->bufx.ptr.p_float[0], 1, &state->xc.ptr.p_float[0], 1, ae_v_len(0,n-1));
            for(i=0; i<=n-1; i++)
            {
                state->tmp0.ptr.p_float[i] = state->activeconstraints.ptr.p_int[i];
            }
            ...
            for(k=0; k<=n-1; k++)
            {
                ...
                {
                    v = state->bufb.ptr.p_float[k];
                    state->bufb.ptr.p_float[k] = state->bufb.ptr.p_float[state->p2.ptr.p_int[k]];
                    state->bufb.ptr.p_float[state->p2.ptr.p_int[k]] = v;
                    v = state->bufx.ptr.p_float[k];
                    state->bufx.ptr.p_float[k] = state->bufx.ptr.p_float[state->p2.ptr.p_int[k]];
                    state->bufx.ptr.p_float[state->p2.ptr.p_int[k]] = v;
                }
            }
            for(i=0; i<=n-1; i++)
            {
                ae_v_move(&state->tmp0.ptr.p_float[0], 1, &state->bufa.ptr.pp_float[i][0], 1, ae_v_len(0,n-1));
                for(k=0; k<=n-1; k++)
                {
                    if( state->p2.ptr.p_int[k]!=k )
                    {
                        v = state->tmp0.ptr.p_float[k];
                        state->tmp0.ptr.p_float[k] = state->tmp0.ptr.p_float[state->p2.ptr.p_int[k]];
                        state->tmp0.ptr.p_float[state->p2.ptr.p_int[k]] = v;
                    }
                }
                ae_v_move(&state->bufa.ptr.pp_float[i][0], 1, &state->tmp0.ptr.p_float[0], 1, ae_v_len(0,n-1));
            }
            for(i=0; i<=n-1; i++)
            {
                ...
                {
                    ae_v_move(&state->tmp0.ptr.p_float[0], 1, &state->bufa.ptr.pp_float[i][0], 1, ae_v_len(0,n-1));
                    ae_v_move(&state->bufa.ptr.pp_float[i][0], 1, &state->bufa.ptr.pp_float[state->p2.ptr.p_int[i]][0], 1, ae_v_len(0,n-1));
                    ae_v_move(&state->bufa.ptr.pp_float[state->p2.ptr.p_int[i]][0], 1, &state->tmp0.ptr.p_float[0], 1, ae_v_len(0,n-1));
                }
            }
            ...
            ae_v_add(&state->bufb.ptr.p_float[0], 1, &state->tmp0.ptr.p_float[0], 1, ae_v_len(0,nfree-1));
            state->constterm = 0.0;
            for(i=nfree; i<=n-1; i++)
            {
                state->constterm = state->constterm+0.5*state->bufx.ptr.p_float[i]*state->bufa.ptr.pp_float[i][i]*state->bufx.ptr.p_float[i];
                for(j=i+1; j<=n-1; j++)
                {
                    state->constterm = state->constterm+state->bufx.ptr.p_float[i]*state->bufa.ptr.pp_float[i][j]*state->bufx.ptr.p_float[j];
                }
            }
            ...
            ae_v_moveneg(&state->bufx.ptr.p_float[0], 1, &state->bufb.ptr.p_float[0], 1, ae_v_len(0,nfree-1));
            ...
            ae_v_move(&state->xc.ptr.p_float[0], 1, &state->bufx.ptr.p_float[0], 1, ae_v_len(0,n-1));
            for(i=0; i<=n-1; i++)
            {
                if( state->havebndl.ptr.p_bool[i] )
                {
             ...
             */
            f = ae_v_dotproduct(&state->b.ptr.p_float[0], 1, &state->xc.ptr.p_float[0], 1, ae_v_len(0,n-1));
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&x->ptr.p_float[0], 1, &state->xc.ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    rep->inneriterationscount = state->repinneriterationscount;
    rep->outeriterationscount = state->repouteriterationscount;
     ...
     ae_state *_state)
{
    ...
    n = state->n;
    ae_v_move(&state->b.ptr.p_float[0], 1, &b->ptr.p_float[0], 1, ae_v_len(0,n-1));
}
     ...
     ae_state *_state)
{
    ...
    if( isupper )
    {
        for(k=0; k<=n-2; k++)
        {
            state->diaga.ptr.p_float[k] = a->ptr.pp_float[k][k]+s;
            ae_v_move(&state->densea.ptr.pp_float[k+1][k], state->densea.stride, &a->ptr.pp_float[k][k+1], 1, ae_v_len(k+1,n-1));
        }
        state->diaga.ptr.p_float[n-1] = a->ptr.pp_float[n-1][n-1]+s;
    }
    else
    {
        state->diaga.ptr.p_float[0] = a->ptr.pp_float[0][0]+s;
        for(k=1; k<=n-1; k++)
        {
            ae_v_move(&state->densea.ptr.pp_float[k][0], 1, &a->ptr.pp_float[k][0], 1, ae_v_len(0,k-1));
            state->diaga.ptr.p_float[k] = a->ptr.pp_float[k][k]+s;
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(k=0; k<=n-1; k++)
    {
        state->diaga.ptr.p_float[k] = s->ptr.p_float[k];
    }
}
     ...
     ae_state *_state)
{
    ...
    n = state->n;
    ae_v_move(&state->startx.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->havex = ae_true;
}
     ...
     ae_state *_state)
{
    ...
    n = state->n;
    ae_v_move(&state->xorigin.ptr.p_float[0], 1, &xorigin->ptr.p_float[0], 1, ae_v_len(0,n-1));
}
...
static void minqp_minqpgrad(minqpstate* state, ae_state *_state)
{
    ...
    if( state->akind==-1 )
    {
        ae_v_move(&state->gc.ptr.p_float[0], 1, &state->b.ptr.p_float[0], 1, ae_v_len(0,n-1));
        return;
    }
    ...
    {
        ae_v_move(&state->gc.ptr.p_float[0], 1, &state->b.ptr.p_float[0], 1, ae_v_len(0,n-1));
        state->gc.ptr.p_float[0] = state->gc.ptr.p_float[0]+state->diaga.ptr.p_float[0]*state->xc.ptr.p_float[0];
        for(i=1; i<=n-1; i++)
        {
            v = ae_v_dotproduct(&state->densea.ptr.pp_float[i][0], 1, &state->xc.ptr.p_float[0], 1, ae_v_len(0,i-1));
            state->gc.ptr.p_float[i] = state->gc.ptr.p_float[i]+v+state->diaga.ptr.p_float[i]*state->xc.ptr.p_float[i];
            v = state->xc.ptr.p_float[i];
            ae_v_addd(&state->gc.ptr.p_float[0], 1, &state->densea.ptr.pp_float[i][0], 1, ae_v_len(0,i-1), v);
        }
        return;
    }
}
     ...
     ae_state *_state)
{
    ...
    if( state->akind==0 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            for(j=0; j<=i-1; j++)
            {
                result = result+state->densea.ptr.pp_float[i][j]*x->ptr.p_float[i]*x->ptr.p_float[j];
            }
            result = result+0.5*state->diaga.ptr.p_float[i]*ae_sqr(x->ptr.p_float[i], _state);
        }
        return result;
    }
    return result;
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        ae_assert(ae_isfinite(s->ptr.p_float[i], _state), "MinLMSetScale: S contains infinite or NAN elements", _state);
        ae_assert(ae_fp_neq(s->ptr.p_float[i],0), "MinLMSetScale: S contains zero elements", _state);
        state->s.ptr.p_float[i] = ae_fabs(s->ptr.p_float[i], _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ae_assert(ae_isfinite(bndl->ptr.p_float[i], _state)||ae_isneginf(bndl->ptr.p_float[i], _state), "MinLMSetBC: BndL contains NAN or +INF", _state);
        ae_assert(ae_isfinite(bndu->ptr.p_float[i], _state)||ae_isposinf(bndu->ptr.p_float[i], _state), "MinLMSetBC: BndU contains NAN or -INF", _state);
        state->bndl.ptr.p_float[i] = bndl->ptr.p_float[i];
        state->havebndl.ptr.p_bool[i] = ae_isfinite(bndl->ptr.p_float[i], _state);
        state->bndu.ptr.p_float[i] = bndu->ptr.p_float[i];
        state->havebndu.ptr.p_bool[i] = ae_isfinite(bndu->ptr.p_float[i], _state);
...
ae_bool minlmiteration(minlmstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        v = state->rstate.ra.ptr.p_float[0];
        s = state->rstate.ra.ptr.p_float[1];
        t = state->rstate.ra.ptr.p_float[2];
    }
    else
    {
        n = -983;
...
lbl_1:
    ...
    v = ae_v_dotproduct(&state->fi.ptr.p_float[0], 1, &state->fi.ptr.p_float[0], 1, ae_v_len(0,m-1));
    state->f = v;
lbl_19:
...
lbl_28:
     ...
     */
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->x.ptr.p_float[k] = state->x.ptr.p_float[k]-state->s.ptr.p_float[k]*state->diffstep;
    if( state->havebndl.ptr.p_bool[k] )
    {
        state->x.ptr.p_float[k] = ae_maxreal(state->x.ptr.p_float[k], state->bndl.ptr.p_float[k], _state);
    }
    if( state->havebndu.ptr.p_bool[k] )
    {
        state->x.ptr.p_float[k] = ae_minreal(state->x.ptr.p_float[k], state->bndu.ptr.p_float[k], _state);
    }
    state->xm1 = state->x.ptr.p_float[k];
    minlm_clearrequestfields(state, _state);
    state->needfi = ae_true;
    state->rstate.stage = 3;
    goto lbl_rcomm;
lbl_3:
    state->repnfunc = state->repnfunc+1;
    ae_v_move(&state->fm1.ptr.p_float[0], 1, &state->fi.ptr.p_float[0], 1, ae_v_len(0,m-1));
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->x.ptr.p_float[k] = state->x.ptr.p_float[k]+state->s.ptr.p_float[k]*state->diffstep;
    if( state->havebndl.ptr.p_bool[k] )
    {
    ...
    state->xp1 = state->x.ptr.p_float[k];
    minlm_clearrequestfields(state, _state);
    state->needfi = ae_true;
    state->rstate.stage = 4;
    goto lbl_rcomm;
lbl_4:
    state->repnfunc = state->repnfunc+1;
    ae_v_move(&state->fp1.ptr.p_float[0], 1, &state->fi.ptr.p_float[0], 1, ae_v_len(0,m-1));
    v = state->xp1-state->xm1;
    if( ae_fp_neq(v,0) )
    {
        v = 1/v;
        ae_v_moved(&state->j.ptr.pp_float[0][k], state->j.stride, &state->fp1.ptr.p_float[0], 1, ae_v_len(0,m-1), v);
        ae_v_subd(&state->j.ptr.pp_float[0][k], state->j.stride, &state->fm1.ptr.p_float[0], 1, ae_v_len(0,m-1), v);
    }
    else
    {
        for(i=0; i<=m-1; i++)
        {
            state->j.ptr.pp_float[i][k] = 0;
        }
    }
    k = k+1;
    goto lbl_28;
...
lbl_24:
    ...
    t = ae_v_dotproduct(&state->deltax.ptr.p_float[0], 1, &state->deltax.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_assert(ae_fp_neq(t,0), "MinLM: internal error (T=0)", _state);
    for(i=0; i<=m-1; i++)
    {
        v = ae_v_dotproduct(&state->j.ptr.pp_float[i][0], 1, &state->deltax.ptr.p_float[0], 1, ae_v_len(0,n-1));
        v = (state->deltaf.ptr.p_float[i]-v)/t;
        ae_v_addd(&state->j.ptr.pp_float[i][0], 1, &state->deltax.ptr.p_float[0], 1, ae_v_len(0,n-1), v);
    }
    ae_v_move(&state->fi.ptr.p_float[0], 1, &state->fibase.ptr.p_float[0], 1, ae_v_len(0,m-1));
    ae_v_add(&state->fi.ptr.p_float[0], 1, &state->deltaf.ptr.p_float[0], 1, ae_v_len(0,m-1));
...
lbl_25:
    ...
    ae_v_muld(&state->gbase.ptr.p_float[0], 1, ae_v_len(0,n-1), 2);
    v = ae_v_dotproduct(&state->fi.ptr.p_float[0], 1, &state->fi.ptr.p_float[0], 1, ae_v_len(0,m-1));
    state->fbase = v;
    ae_v_move(&state->fibase.ptr.p_float[0], 1, &state->fi.ptr.p_float[0], 1, ae_v_len(0,m-1));
...
lbl_7:
    ...
    ae_v_move(&state->gbase.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->fbase = state->f;
...
lbl_31:
    ...
    if( ae_fp_less(state->lambdav,0) )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            state->lambdav = ae_maxreal(state->lambdav, ae_fabs(state->quadraticmodel.ptr.pp_float[i][i], _state)*ae_sqr(state->s.ptr.p_float[i], _state), _state);
        }
...
lbl_39:
    ...
    for(i=0; i<=n-1; i++)
    {
        state->tmp0.ptr.p_float[i] = state->quadraticmodel.ptr.pp_float[i][i]+state->lambdav/ae_sqr(state->s.ptr.p_float[i], _state);
    }
    ...
    if( state->qprep.terminationtype>0 )
    {
         ...
         */
        ae_v_sub(&state->xdir.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
        v = ae_v_dotproduct(&state->xdir.ptr.p_float[0], 1, &state->xdir.ptr.p_float[0], 1, ae_v_len(0,n-1));
     ...
     */
    ae_v_move(&state->deltax.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_add(&state->deltax.ptr.p_float[0], 1, &state->xdir.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_sub(&state->deltax.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->deltaxready = ae_true;
    v = 0.0;
    for(i=0; i<=n-1; i++)
    {
        v = v+ae_sqr(state->deltax.ptr.p_float[i]/state->s.ptr.p_float[i], _state);
    }
    v = ae_sqrt(v, _state);
...
lbl_41:
    ...
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->xbase.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_add(&state->x.ptr.p_float[0], 1, &state->xdir.ptr.p_float[0], 1, ae_v_len(0,n-1));
    minlm_clearrequestfields(state, _state);
    if( !state->hasfi )
    {
        goto lbl_47;
...
lbl_10:
    ...
    v = ae_v_dotproduct(&state->fi.ptr.p_float[0], 1, &state->fi.ptr.p_float[0], 1, ae_v_len(0,m-1));
    state->f = v;
    ae_v_move(&state->deltaf.ptr.p_float[0], 1, &state->fi.ptr.p_float[0], 1, ae_v_len(0,m-1));
    ae_v_sub(&state->deltaf.ptr.p_float[0], 1, &state->fibase.ptr.p_float[0], 1, ae_v_len(0,m-1));
...
lbl_40:
    ...
    for(i=0; i<=n-1; i++)
    {
        v = ae_v_dotproduct(&state->quadraticmodel.ptr.pp_float[i][0], 1, &state->deltax.ptr.p_float[0], 1, ae_v_len(0,n-1));
        t = t+state->deltax.ptr.p_float[i]*state->gbase.ptr.p_float[i]+0.5*state->deltax.ptr.p_float[i]*v;
    }
...
lbl_49:
     ...
     */
    ae_v_add(&state->xbase.ptr.p_float[0], 1, &state->deltax.ptr.p_float[0], 1, ae_v_len(0,n-1));
    if( !state->xrep )
    {
        goto lbl_55;
    }
...
lbl_rcomm:
    ...
    state->rstate.ra.ptr.p_float[0] = v;
    state->rstate.ra.ptr.p_float[1] = s;
    state->rstate.ra.ptr.p_float[2] = t;
    return result;
}
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&state->xbase.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    ae_vector_set_length(&state->rstate.ia, 4+1, _state);
    ae_vector_set_length(&state->rstate.ba, 0+1, _state);
    ae_vector_set_length(&state->rstate.ra, 2+1, _state);
    state->rstate.stage = -1;
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        state->bndl.ptr.p_float[i] = _state->v_neginf;
        state->havebndl.ptr.p_bool[i] = ae_false;
        state->bndu.ptr.p_float[i] = _state->v_posinf;
        state->havebndu.ptr.p_bool[i] = ae_false;
    }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        v = -g->ptr.p_float[i]*state->s.ptr.p_float[i];
        if( state->havebndl.ptr.p_bool[i] )
        {
            if( ae_fp_less_eq(x->ptr.p_float[i],state->bndl.ptr.p_float[i])&&ae_fp_less(-g->ptr.p_float[i],0) )
            {
                v = 0;
            }
        }
        if( state->havebndu.ptr.p_bool[i] )
        {
            if( ae_fp_greater_eq(x->ptr.p_float[i],state->bndu.ptr.p_float[i])&&ae_fp_greater(-g->ptr.p_float[i],0) )
            {
                v = 0;
            }
        }
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        ae_assert(ae_fp_less_eq(bndl->ptr.p_float[i],bndu->ptr.p_float[i]), "MinASA: inconsistent bounds!", _state);
        ae_assert(ae_fp_less_eq(bndl->ptr.p_float[i],x->ptr.p_float[i]), "MinASA: infeasible X!", _state);
        ae_assert(ae_fp_less_eq(x->ptr.p_float[i],bndu->ptr.p_float[i]), "MinASA: infeasible X!", _state);
    }
...
ae_bool minasaiteration(minasastate* state, ae_state *_state)
{
    ...
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_eq(state->xk.ptr.p_float[i],state->bndl.ptr.p_float[i])||ae_fp_eq(state->xk.ptr.p_float[i],state->bndu.ptr.p_float[i]) )
        {
            state->ak.ptr.p_float[i] = 0;
        }
        else
        {
            state->ak.ptr.p_float[i] = 1;
        }
    }
    state->mu = 0.1;
    state->curalgo = 0;
...
lbl_21:
    ...
    for(i=0; i<=n-1; i++)
    {
        state->d.ptr.p_float[i] = boundval(state->xk.ptr.p_float[i]-state->g.ptr.p_float[i], state->bndl.ptr.p_float[i], state->bndu.ptr.p_float[i], _state)-state->xk.ptr.p_float[i];
    }
     ...
     * not bounded.
     */
    v = ae_v_dotproduct(&state->d.ptr.p_float[0], 1, &state->g.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->dginit = v;
    state->finit = state->f;
    ...
    for(i=0; i<=n-1; i++)
    {
        state->x.ptr.p_float[i] = boundval(state->xk.ptr.p_float[i]-state->g.ptr.p_float[i], state->bndl.ptr.p_float[i], state->bndu.ptr.p_float[i], _state);
    }
    mincomp_clearrequestfields(state, _state);
    state->needfg = ae_true;
    state->rstate.stage = 2;
...
lbl_27:
    ...
    v = state->stp;
    ae_v_move(&state->x.ptr.p_float[0], 1, &state->xk.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_addd(&state->x.ptr.p_float[0], 1, &state->d.ptr.p_float[0], 1, ae_v_len(0,n-1), v);
    mincomp_clearrequestfields(state, _state);
    state->needfg = ae_true;
    state->rstate.stage = 3;
    goto lbl_rcomm;
...
lbl_29:
    ...
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_eq(state->xn.ptr.p_float[i],state->bndl.ptr.p_float[i])||ae_fp_eq(state->xn.ptr.p_float[i],state->bndu.ptr.p_float[i]) )
        {
            state->an.ptr.p_float[i] = 0;
        }
        else
        {
            state->an.ptr.p_float[i] = 1;
        }
    }
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_neq(state->ak.ptr.p_float[i],state->an.ptr.p_float[i]) )
        {
            state->acount = -1;
            break;
        }
    ...
    ae_v_move(&state->xk.ptr.p_float[0], 1, &state->xn.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_move(&state->ak.ptr.p_float[0], 1, &state->an.ptr.p_float[0], 1, ae_v_len(0,n-1));
...
lbl_37:
    ...
    return result;
lbl_35:
    v = ae_v_dotproduct(&state->d.ptr.p_float[0], 1, &state->d.ptr.p_float[0], 1, ae_v_len(0,n-1));
...
lbl_19:
    ...
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_neq(state->ak.ptr.p_float[i],0) )
        {
            b = ae_false;
            break;
        }
    ...
    for(i=0; i<=n-1; i++)
    {
        state->dk.ptr.p_float[i] = -state->g.ptr.p_float[i]*state->ak.ptr.p_float[i];
        state->gc.ptr.p_float[i] = state->g.ptr.p_float[i]*state->ak.ptr.p_float[i];
    }
lbl_49:
    if( ae_false )
    {
    ...
    for(i=0; i<=n-1; i++)
    {
        state->yk.ptr.p_float[i] = -state->gc.ptr.p_float[i];
    }
...
lbl_51:
    ...
    for(i=0; i<=n-1; i++)
    {
        state->x.ptr.p_float[i] = boundval(state->xn.ptr.p_float[i], state->bndl.ptr.p_float[i], state->bndu.ptr.p_float[i], _state);
    }
...
lbl_9:
    ...
    for(i=0; i<=n-1; i++)
    {
        if( ae_fp_eq(state->x.ptr.p_float[i],state->bndl.ptr.p_float[i])||ae_fp_eq(state->x.ptr.p_float[i],state->bndu.ptr.p_float[i]) )
        {
            state->gc.ptr.p_float[i] = 0;
        }
        else
        {
            state->gc.ptr.p_float[i] = state->g.ptr.p_float[i];
        }
    }
    mcsrch(n, &state->xn, &state->f, &state->gc, &state->d, &state->stp, state->stpmax, mincomp_gtol, &mcinfo, &state->nfev, &state->work, &state->lstate, &state->mcstage, _state);
    goto lbl_51;
lbl_52:
    ...
    for(i=0; i<=n-1; i++)
    {
         ...
         */
        state->xn.ptr.p_float[i] = boundval(state->xn.ptr.p_float[i], state->bndl.ptr.p_float[i], state->bndu.ptr.p_float[i], _state);
        ...
        if( ae_fp_neq(state->an.ptr.p_float[i],state->ak.ptr.p_float[i]) )
        {
            diffcnt = diffcnt+1;
        }
        state->ak.ptr.p_float[i] = state->an.ptr.p_float[i];
    }
    ae_v_move(&state->xk.ptr.p_float[0], 1, &state->xn.ptr.p_float[0], 1, ae_v_len(0,n-1));
    state->repnfev = state->repnfev+state->nfev;
    state->repiterationscount = state->repiterationscount+1;
...
lbl_63:
    ...
    if( mcinfo==1 )
    {
         ...
         */
        ae_v_add(&state->yk.ptr.p_float[0], 1, &state->gc.ptr.p_float[0], 1, ae_v_len(0,n-1));
        vv = ae_v_dotproduct(&state->yk.ptr.p_float[0], 1, &state->dk.ptr.p_float[0], 1, ae_v_len(0,n-1));
        v = ae_v_dotproduct(&state->gc.ptr.p_float[0], 1, &state->gc.ptr.p_float[0], 1, ae_v_len(0,n-1));
        state->betady = v/vv;
        v = ae_v_dotproduct(&state->gc.ptr.p_float[0], 1, &state->yk.ptr.p_float[0], 1, ae_v_len(0,n-1));
        state->betahs = v/vv;
        if( state->cgtype==0 )
        {
            betak = state->betady;
    ...
    ae_v_moveneg(&state->dn.ptr.p_float[0], 1, &state->gc.ptr.p_float[0], 1, ae_v_len(0,n-1));
    ae_v_addd(&state->dn.ptr.p_float[0], 1, &state->dk.ptr.p_float[0], 1, ae_v_len(0,n-1), betak);
    ae_v_move(&state->dk.ptr.p_float[0], 1, &state->dn.ptr.p_float[0], 1, ae_v_len(0,n-1));
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        if( ae_fp_eq(state->ak.ptr.p_float[i],0) )
        {
     ...
     ae_state *_state)
{
    ...
    ae_v_move(&state->x.ptr.p_float[0], 1, &x->ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    ae_v_move(&state->bndl.ptr.p_float[0], 1, &bndl->ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    ae_v_move(&state->bndu.ptr.p_float[0], 1, &bndu->ptr.p_float[0], 1, ae_v_len(0,state->n-1));
    state->laststep = 0;
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        v = -state->g.ptr.p_float[i];
        if( ae_fp_eq(state->x.ptr.p_float[i],state->bndl.ptr.p_float[i])&&ae_fp_less(-state->g.ptr.p_float[i],0) )
        {
            v = 0;
        }
        if( ae_fp_eq(state->x.ptr.p_float[i],state->bndu.ptr.p_float[i])&&ae_fp_greater(-state->g.ptr.p_float[i],0) )
        {
            v = 0;
        }
        result = result+ae_sqr(v, _state);
...
static float mincomp_asaginorm(minasastate* state, ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        if( ae_fp_neq(state->x.ptr.p_float[i],state->bndl.ptr.p_float[i])&&ae_fp_neq(state->x.ptr.p_float[i],state->bndu.ptr.p_float[i]) )
        {
            result = result+ae_sqr(state->g.ptr.p_float[i], _state);
        }
    }
    result = ae_sqrt(result, _state);
    return result;
...
static float mincomp_asad1norm(minasastate* state, ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        result = result+ae_sqr(boundval(state->x.ptr.p_float[i]-state->g.ptr.p_float[i], state->bndl.ptr.p_float[i], state->bndu.ptr.p_float[i], _state)-state->x.ptr.p_float[i], _state);
    }
    result = ae_sqrt(result, _state);
    return result;
}
...
static ae_bool mincomp_asauisempty(minasastate* state, ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        if( ae_fp_greater_eq(ae_fabs(state->g.ptr.p_float[i], _state),d2)&&ae_fp_greater_eq(ae_minreal(state->x.ptr.p_float[i]-state->bndl.ptr.p_float[i], state->bndu.ptr.p_float[i]-state->x.ptr.p_float[i], _state),d32) )
        {
            result = ae_false;
            return result;
        }
