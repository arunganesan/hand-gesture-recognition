...
int main (int argc, char * argv []) {
  if (argc != 3) {
    cout << "Usage: ./<exec> <nrange> <ntraining images>" << endl;
    exit(1);
  }
  ...
  sprintf(model_file, "RF.range.%d.%d.model", nrange, ntrain_images);
  sprintf(training_file, "Range%d.%d.features.txt", nrange, ntrain_images);
  sprintf(test_file, "Range%d.test.features.txt", nrange, nfeatures);
  sprintf(results_file, "Results.range.%d.%d.txt", nrange, ntrain_images);
