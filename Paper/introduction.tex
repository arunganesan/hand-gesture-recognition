...
\textbf{An inexpensive way to generate labeled data.} We color gloves to easily generate labeled data through the use of alignment of RGB and depth images. In \todo{MSR}, the authors use sophisticated computer graphics to generate training samples. We found this expensive and through the use of color gloves, developers can generate their customized gesture without difficulty. Another advantage of our approach is that the system use raw depth image as the training samples considering the depth shadowing and many other realistic noisy effect. In \todo{MSR} however it is very difficult to simulate the above mentioned noisy effects. 
