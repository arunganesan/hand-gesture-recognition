...
void normalize(problem * prob)
{
	...
	for (i=0; i<prob->l; i++)
	{
		...
		if (sum_norm> 1e-60){
			...
			while(xi->index != -1)
			{
				xi->value = xi->value/sum_norm;
				xi++;
			}
		}else
            printf ("See a all-zero vector %d\n", i);
        // otherwise ignore the all zero vector
...
static void solve_l2r_l1l2_svc(
	...
	double Cp, double Cn, int solver_type, int max_iter, const parameter * param, int class_index = 0)
{
	int l = prob->l;
	int w_size = prob->n;
	int i, s, iter = 0;
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn, int class_index=0)
{
	double eps=param->eps;
	int pos = 0;
	int neg = 0;
	if (param->multicore_flag==0)
	{
	...
	switch(param->solver_type)
	{
		...
		case L2R_L2LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL, param->max_iter, param, class_index);
			break;
		case L2R_L1LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL, param->max_iter, param, class_index);
			break;
		case L1R_L2LOSS_SVC:
		{
			problem prob_col;
...
model* train(const problem *prob, const parameter *param)
{
	...
	else
	{
		...
		else
		{
			...
			if (param -> multicore_flag == 1)
            {
                sub_prob.y_mp = new int * [nr_class];
                #pragma omp parallel for
                for(i=0;i<nr_class;i++)
                {
                    //printf ("Multithread starts \n");
                    int si = start[i];
                    int ei = si+count[i];
                    sub_prob.y_mp[i] = new int [sub_prob.l];
                    int k=0;  // This is the crux!
                    for(; k<si; k++)
                        //sub_prob.y[k] = -1;
                        sub_prob.y_mp[i][k] = -1;
                    for(; k<ei; k++)
                    ...
                    for(; k<sub_prob.l; k++)
                        //sub_prob.y[k] = -1;
                        sub_prob.y_mp[i][k] = -1;
                    train_one(&sub_prob, param, model_->w+i*w_size, weighted_C[i], param->C, i);		               
                }
            }
            else
            {
...
int predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	for(; (idx=lx->index)!=-1; lx++)
	{
		...
		if(idx<=n)
			for(i=0;i<nr_w;i++)
				dec_values[i] += w[i* n +(idx-1)]*lx->value; // Change
				//dec_values[i] += w[(idx-1)*nr_w+i]*lx->value; // maybe need to change
	}
...
int save_model(const char *model_file_name, const struct model *model_)
{
	...
	for(i=0; i<w_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
			fprintf(fp, "%.16g ", model_->w[j*w_size + i]); // Changed
			//fprintf(fp, "%.16g ", model_->w[i*nr_w+j]); // Needs to change here
		fprintf(fp, "\n");
	}
...
struct model *load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"normalize_flag")==0)
		{ 
			fscanf(fp,"%d",&param.normalize_flag);
		} 
		else
		{
	...
	for(i=0; i<w_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
			fscanf(fp, "%lf ", &model_->w[j*w_size + i]); // Change
			//fscanf(fp, "%lf ", &model_->w[i*nr_w+j]); // Need to change that
		fscanf(fp, "\n");
	}
